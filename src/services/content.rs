...
mod tests {
    ...
    impl ContentRepository for MockContentRepo {
        async fn get_contents(&self, _p: Option<&str>, _l: Option<i64>) -> AppResult<Vec<Content>> {
            Ok(vec![])
        }
        async fn get_by_slug(&self, _s: &str) -> AppResult<Option<Content>> {
            Ok(None)
        }
        ...
        async fn get_similar(&self, _c: &Content) -> AppResult<Vec<Content>> {
            Ok(vec![])
        }
        async fn available_tags(&self) -> AppResult<Vec<String>> {
            Ok(vec![])
        }
        async fn tagged_series(&self, _t: Option<&str>) -> AppResult<Vec<TaggedSeries>> {
            Ok(vec![])
        }
    }
    ...
    impl UserRepository for MockUserRepo {
        async fn create_user(&self, _e: &str, _h: &str) -> AppResult<i64> {
            Ok(1)
        }
        async fn get_by_email(&self, _e: &str) -> AppResult<Option<(i64, String)>> {
            Ok(None)
        }
