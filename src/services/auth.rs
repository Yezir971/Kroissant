...
impl AuthService for AuthServiceImpl {
    async fn register(&self, email: &str, password: &str) -> AppResult<String> {
        ...
        if email.is_empty() || password.len() < 8 {
            return Err(AppError::Auth("Email invalide ou mot de passe trop court (min 8 car.)".to_string()));
        }
    ...
    async fn login(&self, email: &str, password: &str) -> AppResult<String> {
        ...
        let Some((id, hash)) = self.user_repo.get_by_email(&email).await? else {
            return Err(AppError::Auth("Email ou mot de passe incorrect".to_string()));
        };
...
mod tests {
    ...
    impl UserRepository for MockUserRepo {
        ...
        async fn is_saved(&self, _u: i64, _c: i64) -> AppResult<bool> { Ok(false) }
