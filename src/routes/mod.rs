...
pub fn create_router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            ...
            get(handlers::home::privacy),
        )
        .route("/conditions-utilisation", get(handlers::home::terms))
        // PWA
        .route(
            "/manifest.json",
            get_service(ServeFile::new("static/manifest.json")),
        )
        ...
        .route(
            "/inscription",
            get(handlers::registration::registration_shell)
                .post(handlers::auth::register_deprecated),
        )
        .route(
            "/inscription/verify",
            get(handlers::registration::verify_email),
        )
        .route(
            "/inscription/step/email",
            get(handlers::registration::get_step_email).post(handlers::registration::post_email),
        )
        .route(
            "/inscription/step/password",
            get(handlers::registration::get_step_password)
                .post(handlers::registration::post_password),
        )
        .route(
            "/inscription/step/profile",
            get(handlers::registration::get_step_profile)
                .post(handlers::registration::post_profile),
        )
        .route(
            "/inscription/step/{step}",
            get(handlers::registration::get_step),
        )
        .route(
            "/connexion",
